...
register_activation_hook( WC_ADMIN_PLUGIN_FILE, 'wc_admin_activate_plugin' );

/**
 * Deactivate wc-admin plugin if dependencies not satisfied.
 */
function wc_admin_possibly_deactivate_plugin() {
	if ( ! wc_admin_dependencies_satisfied() ) {
		deactivate_plugins( plugin_basename( WC_ADMIN_PLUGIN_FILE ) );
		unset( $_GET['activate'] );
	}
}
add_action( 'admin_init', 'wc_admin_possibly_deactivate_plugin' );

/**
 * On deactivating the wc-admin plugin.
 */
function wc_admin_deactivate_plugin() {
	wp_clear_scheduled_hook( 'wc_admin_daily' );
	WC_Admin_Reports_Sync::clear_queued_actions();
}
register_deactivation_hook( WC_ADMIN_PLUGIN_FILE, 'wc_admin_deactivate_plugin' );
