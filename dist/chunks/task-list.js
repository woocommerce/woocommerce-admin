...
var sanitize_html = __webpack_require__(507);
...
const Marketing_Marketing = () => {
  ...
  const transformExtensionToPlugin = extension => {
    const {
      description,
      image_url,
      key,
  ...
  const [installedExtensions, pluginLists] = Object(external_wp_element_["useMemo"])(() => {
    const installed = [];
    const lists = [];
...
function getAllTasks({
  ...
  const tasks = [{
    ...
    completed: [].filter(plugin => installedPlugins.includes(plugin)).length > 0,
    visible: window.wcAdminFeatures && window.wcAdminFeatures['remote-extensions-list'],
    time: Object(external_wp_i18n_["__"])('1 minute', 'woocommerce-admin'),
    type: 'setup'
  }, {
    key: 'appearance',
