...
class WC_Tests_Mailchimp_Scheduler extends WC_Unit_Test_Case {
	...
	public function setUp() {
		$this->logger_mcok = $this->getMockBuilder( \WC_Logger_Interface::class )
			->getMock();
	...
	public function test_it_logs_error_when_wp_error_is_returned() {
		...
		$this->logger_mcok->expects( $this->exactly( 2 ) )
			->method( 'error' )
			->with( 'Error getting a response from Mailchimp API.', MailchimpScheduler::LOGGER_CONTEXT );
	...
	public function test_it_logs_error_when_response_data_is_incorrect() {
		...
		$this->logger_mcok->expects( $this->once() )
						...
						->with(
							// phpcs:ignore
							'Incorrect response from Mailchimp API: ' . print_r( array(), true ),
							MailchimpScheduler::LOGGER_CONTEXT
						);
