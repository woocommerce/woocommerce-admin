...
describe( 'Store owner can enable or disable onboarding', () => {
	...
	it( 'enables the new onboarding experience', async () => {
		...
		await expect( page ).toClick( '#tab-link-woocommerce_onboard_tab a', {
			text: 'Setup wizard',
		} );

		await Promise.all( [
			page.waitForNavigation( { waitUntil: 'networkidle0' } ),
			expect( page ).toClick( '#tab-panel-woocommerce_onboard_tab a', {
				text: 'Enable',
			} ),
		] );
	...
	it( 'walks through the profiler', async () => {
		...
		await expect( page ).toMatchElement(
			'.woocommerce-profile-wizard__usage-modal'
		);
		await expect( page ).toClick( 'button', { text: 'Continue' } );

		// Skip the Jetpack connection for testing purposes.
		await page.goto(
			baseUrl + 'wp-admin/admin.php?page=wc-admin&step=store-details',
			{
				waitUntil: 'networkidle0',
			}
		);
		...
		await expect( page ).toClick(
			'.woocommerce-profile-wizard__checkbox:first-child input'
		);
		await expect( page ).toClick( 'button', { text: 'Continue' } );
		...
		await expect( page ).toClick(
			'.woocommerce-profile-wizard__checkbox:nth-child(2) input'
		);
		await expect( page ).toClick( 'button', { text: 'Continue' } );
		...
		await Promise.all( [
			...
			expect( page ).toMatchElement(
				'.woocommerce-card__body div:first-child .woocommerce-select-control__control'
			),
		] );
		await expect( page ).toClick(
			'.woocommerce-card__body div:first-child .woocommerce-select-control__control'
		...
		await expect( page ).toClick(
			'#woocommerce-select-control__option-1-0'
		);
		await expect( page ).toClick(
			'.woocommerce-card__body div:nth-child(2) .woocommerce-select-control__control'
		);
		await expect( page ).toClick(
			'#woocommerce-select-control__option-2-no'
		);
		await expect( page ).toClick( 'button', { text: 'Continue' } );
		...
		await expect(
			page
		).toClick( '.woocommerce-profile-wizard__theme:nth-child(2) button', {
			text: 'Choose',
		} );
		...
		await expect( page ).toMatchElement(
			'body:not(.woocommerce-profile-wizard__body)'
		);
	} );
} );
