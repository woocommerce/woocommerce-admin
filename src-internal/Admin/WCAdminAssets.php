...
class WCAdminAssets {
	 ...
	 */
	private function should_use_minified_js_file( $script_debug ) {
	 ...
	 */
	private function get_url( $file, $ext ) {
		$suffix = '';

		// Potentially enqueue minified JavaScript.
		if ( 'js' === $ext ) {
			$script_debug = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG;
			$suffix       = $this->should_use_minified_js_file( $script_debug ) ? '.min' : '';
		}

		return plugins_url( $this->get_path( $ext ) . $file . $suffix . '.' . $ext, WC_ADMIN_PLUGIN_FILE );
	}
	 ...
	 */
	private function get_file_version( $ext ) {
		if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
			return filemtime( WC_ADMIN_ABSPATH . $this->get_path( $ext ) );
		}
		return WC_VERSION;
	}
	 ...
	 */
	private function get_script_asset_filename( $script_path_name, $file ) {
	...
	public function register_scripts() {
		...
		$js_file_version  = $this->get_file_version( 'js' );
		$css_file_version = $this->get_file_version( 'css' );
		...
		foreach ( $scripts as $script ) {
			...
			try {
				$script_assets_filename = $this->get_script_asset_filename( $script_path_name, 'index' );
				$script_assets          = require WC_ADMIN_ABSPATH . WC_ADMIN_DIST_JS_FOLDER . $script_path_name . '/' . $script_assets_filename;

				wp_register_script(
					$script,
					$this->get_url( $script_path_name . '/index', 'js' ),
		...
		wp_register_style(
			'wc-components',
			$this->get_url( 'components/style', 'css' ),
			array(),
			$css_file_version
		);
		...
		wp_register_style(
			'wc-customer-effort-score',
			$this->get_url( 'customer-effort-score/style', 'css' ),
			array(),
			$css_file_version
		);
		...
		wp_register_style(
			'wc-experimental',
			$this->get_url( 'experimental/style', 'css' ),
			array(),
			$css_file_version
		);
		...
		wp_localize_script(
			...
			'wcAdminAssets',
			array(
				'path'    => plugins_url( $this->get_path( 'js' ), WC_ADMIN_PLUGIN_FILE ),
		...
		wp_register_style(
			'wc-onboarding',
			$this->get_url( 'onboarding/style', 'css' ),
			array(),
			$css_file_version
		);
