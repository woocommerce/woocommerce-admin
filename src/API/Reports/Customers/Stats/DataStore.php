...
class DataStore extends CustomersDataStore implements DataStoreInterface {
	...
	public function get_data( $query_args ) {
		...
		if ( false === $data ) {
			...
			$this->subquery->add_sql_clause(
				'select',
				'CASE WHEN SUM( CASE WHEN parent_id = 0 THEN 1 ELSE 0 END ) = 0 THEN NULL ELSE SUM( gross_total ) / SUM( CASE WHEN parent_id = 0 THEN 1 ELSE 0 END ) END AS avg_order_value'
			);
